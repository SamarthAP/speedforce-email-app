...
export default function FeedbackTab () {
  ...
  const truncateFileName = (fileName: string, maxLength: number) => {
    if (fileName.length > maxLength) {
      ...
      return `${fileNameWithoutExtension.slice(0, maxLength - 4)}...${extension}`;
    }
    return fileName;
  };

  const removeAttachment = (index: number, event: React.MouseEvent<HTMLButtonElement>) => {
  ...
  return (
    <>
      <div className="w-full h-full cursor-pointer flex justify-center p-3" onClick={() => {openModal()}}>          
        <p className='rotate-90 py-3.5 text-xs text-white dark:text-black'>Feedback</p>
      </div>
      {isModalOpen && (
        <div className="fixed inset-0 flex items-center justify-center z-40">
          <div className="modal-bg fixed inset-0 bg-white dark:bg-black opacity-60" onClick={closeModal}></div>
          <form action="#" className="relative w-96">
            <XMarkIcon className="text-gray-500 dark:text-white w-5 h-5 absolute right-0 -top-6 cursor-pointer" onClick={closeModal} />
            ...
            <div className="pl-3 flex justify-start bg-gray-600 dark:bg-slate-100">
              <div className="flex h-6 items-center">
                <input
                  onClick={() => {setKeepAnon(!keepAnon); console.log(keepAnon)}}
                  id="comments"
              ...
              <div className="ml-1 text-xs leading-6">
                <span id="comments-description" className="text-white dark:text-gray-900 underline">
                  Keep Anonymous
                </span>
              </div>
            </div>
            ...
            <div className="relative">
              <div className="flex items-center justify-between space-x-3 rounded-b-lg border-slate-600 dark:border-slate-200 bg-gray-600 dark:bg-slate-100 px-2 py-2 sm:px-3">
                ...
                <div className="flex flex-col">
                  ...
                  >
                      <PaperClipIcon className="-ml-1 mr-2 h-5 w-5 group-hover:text-blue-300" aria-hidden="true" />
                      <span className="text-sm italic text-gray-50 dark:text-gray-500 group-hover:text-blue-300">Attach a file</span>
                  </button>
                  
                  {attachments.length > 0 && (
                    <>
                      <label className="block pt-3 text-sm text-gray-200 dark:text-gray-500 underline">Attachments:</label>
                      <div className="text-sm italic text-white dark:text-black">
                        <ul>
                          {attachments.map((file, index) => (
                            <li key={index}>
                              <button
                                onClick={(event) => removeAttachment(index, event)}
                ...
                <div  className="flex-shrink-0 absolute right-3 bottom-3">
                  {!isSubmitting ? (
                    <SimpleButton
                      onClick={(event) => {void submitFeedback(event)}}
                      loading={false}
                      text="Submit"
                      width="w-16"
                    />
